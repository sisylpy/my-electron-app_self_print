            ...
            todayOrderList: {
                handler(newVal) {
                    ...
                    if (Array.isArray(newVal) && newVal.length > 0 && !this.orderData) {
                        ...
                        console.log("🖨️ [ApplyThirtyWholePanel] todayOrderList 更新，订单数:", newVal.length, "合计:", this.subtotal, "单号:", this.tradeNo);
                    }
                },
                immediate: false,
                deep: true
            ...
            todayOrderDepArr: {
                handler(newVal) {
                    ...
                    if (this.hasSubDepartments && Array.isArray(newVal) && newVal.length > 0 && !this.orderData) {
                        ...
                        this.tradeNo = this.todayOrderTradeNo || '';
                        if (this.tradeNo) this.generateQRCode();
                        console.log("🖨️ [ApplyThirtyWholePanel] todayOrderDepArr 更新，部门数:", this.departmentsData.length, "合计:", this.subtotal, "单号:", this.tradeNo);
                    }
                },
                immediate: false,
                deep: true
        ...
        async mounted() {
            ...
            if (this.orderData) {
                ...
                if (this.hasSubDepartments && this.todayOrderDepArr?.length > 0) {
                    ...
                    this.subtotalHanzi = this.numberToChinese(parseFloat(this.subtotal.toFixed(1)), false);
                    console.log("🖨️ [ApplyThirtyWholePanel] 使用今日订单子部门数据，部门数:", this.departmentsData.length, "合计:", this.subtotal);
                } else {
                    this.applyArrPrint = Array.isArray(this.todayOrderList) ? [...this.todayOrderList] : [];
                    this.departmentsData = [];
                    this.subtotal = this.applyArrPrint.reduce((sum, o) => sum + parseFloat(o.nxDoSubtotal || 0), 0);
                    this.subtotalHanzi = this.numberToChinese(parseFloat(this.subtotal.toFixed(1)), false);
                    console.log("🖨️ [ApplyThirtyWholePanel] 使用今日订单列表数据，订单数:", this.applyArrPrint.length, "合计:", this.subtotal);
                }
                this.tradeNo = this.todayOrderTradeNo || '';
        ...
        methods: {
            ...
            async printOnly() {
                ...
                if (this.nxDepFatherId === -1 && this.gbDepFatherId === -1 && this.gbBatchId === -1) {
                    ...
                    console.log(`🖨️ [printOnly] 开始打印，订单条数: ${orderCount}，总页数: ${this.printPagesData.length}`);
