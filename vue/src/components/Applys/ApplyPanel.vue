    ...
    props: ['nxDepFatherId', 'nxDepId', 'depName', 'depPrintName',
        ...
        watch: {
            ...
            showCalibrationPanel(newValue) {
                console.log('🔍 [watch showCalibrationPanel] 状态变化:', {
                    newValue,
                    type: typeof newValue,
                    isTrue: newValue === true
                });
                this.$nextTick(() => {
                    const panelElement = document.querySelector('[style*="打印校准设置"]') || 
                                       document.querySelector('h3:contains("打印校准设置")') ||
                                       document.querySelector('.print-calibration-panel');
                    console.log('🔍 [watch showCalibrationPanel] nextTick 后检查面板元素:', {
                        panelElement: panelElement ? '找到' : '未找到',
                        showCalibrationPanel: this.showCalibrationPanel
                    });
                });
            },
        ...
        todayOrderList: {
            handler(newVal) {
                ...
                if (Array.isArray(newVal) && newVal.length > 0 && !this.orderData) {
                    ...
                    console.log("🖨️ [ApplyPanel] todayOrderList 更新，订单数:", newVal.length, "合计:", this.subtotal, "单号:", this.tradeNo);
                }
            },
            immediate: false,
            deep: true
        ...
        todayOrderDepArr: {
            handler(newVal) {
                ...
                if (this.hasSubDepartments && Array.isArray(newVal) && newVal.length > 0 && !this.orderData) {
                    ...
                    this.tradeNo = this.todayOrderTradeNo || '';
                    if (this.tradeNo) this.generateQRCode();
                    console.log("🖨️ [ApplyPanel] todayOrderDepArr 更新，部门数:", this.departmentsData.length, "合计:", this.subtotal, "单号:", this.tradeNo);
                }
            },
            immediate: false,
            deep: true
        ...
        console.log("gbBatchId===", this.gbBatchId);
            ...
            if (this.orderData) {
                ...
                if (this.hasSubDepartments && this.todayOrderDepArr?.length > 0) {
                    ...
                    this.subtotalHanzi = this.numberToChinese(parseFloat(this.subtotal.toFixed(1)), false);
                    console.log("🖨️ [ApplyPanel] 使用今日订单子部门数据，部门数:", this.departmentsData.length, "合计:", this.subtotal);
                } else {
                    this.applyArrPrint = Array.isArray(this.todayOrderList) ? [...this.todayOrderList] : [];
                    this.departmentsData = [];
                    this.subtotal = this.applyArrPrint.reduce((sum, o) => sum + parseFloat(o.nxDoSubtotal || 0), 0);
                    this.subtotalHanzi = this.numberToChinese(parseFloat(this.subtotal.toFixed(1)), false);
                    console.log("🖨️ [ApplyPanel] 使用今日订单列表数据，订单数:", this.applyArrPrint.length, "合计:", this.subtotal);
                }
                this.tradeNo = this.todayOrderTradeNo || '';
        ...
        methods: {
                ...
                const headerHtml = this.isTraceabilityType ? `
                    ...
                    <div class="print-header">
                        <div class="print-header-content">
                            ...
                            <div class="print-right">
                                <div class="print-item" style="font-size: ${scaledHeaderFontSize}px !important;">共${totalPages}页 - 第${pageIndex}页</div>
                                <div class="print-item" style="font-size: ${scaledHeaderFontSize}px !important;">单号: ${this.tradeNo || ''}</div>
                            </div>
                        </div>
                    </div>`;
                ...
                const rowsHtml = (pageData.dualRows || []).map((item, rowIndex) => {
                    ...
                    return `
                        <div class="print-table-row" style="font-size: ${scaledOrderFontSize}px !important;">
                            <div class="print-col print-ten" style="font-size: ${scaledOrderFontSize}px !important;">${seq}</div>
                            <div class="print-col print-thirty" style="font-size: ${scaledOrderFontSize}px !important;">${name}</div>
                            <div class="print-col print-fifteen" style="font-size: ${scaledOrderFontSize}px !important;">${pack}</div>
                            <div class="print-col print-ten" style="font-size: ${scaledOrderFontSize}px !important;">${std}</div>
                            <div class="print-col print-ten" style="font-size: ${scaledOrderFontSize}px !important;">${qty}</div>
                            <div class="print-col print-ten" style="font-size: ${scaledOrderFontSize}px !important;">${price}</div>
                            <div class="print-col print-fifteen" style="font-size: ${scaledOrderFontSize}px !important;">${sub}</div>
                        </div>`;
                }).join('');
            ...
            async injectPrintStyles(singlePageHtml) {
                ...
                // 如果容器宽度超过物理纸张宽度，按比例缩小到240mm（避免超出纸张）
                    ...
                    <style>
                        ...
                        @media print {
                            ...
                            * { visibility: visible !important; }
                            ...
                            .print-item { 
                                font-size: var(--header-font-size, 14px) !important; 
                            ...
                            .print-col { padding: 0 !important; box-sizing: border-box !important; display: flex !important; align-items: center !important; justify-content: center !important; font-size: var(--order-content-font-size, 14px) !important; line-height: var(--line-height, 24px) !important; white-space: nowrap !important; overflow: hidden !important; text-overflow: ellipsis !important; flex-shrink: 0 !important; flex-grow: 0 !important; }
                            ...
                            .print-col.print-twenty  { flex-basis: 15%;  max-width: 15%; text-align: right !important; padding-right: 2mm !important; }
                            ...
                            .print-subtotal { text-align: right; font-size: var(--order-content-font-size, 14px) !important; font-weight: bold; margin-bottom: 4px; padding: 4px 8px; border-left: 1px solid #000; border-right: 1px solid #000; border-bottom: 1px solid #000; box-sizing: border-box; overflow: visible !important; white-space: nowrap !important; }
            ...
            handleHeaderClick() {
                ...
                if (this.headerClickCount >= 3) {
                    console.log('🔍 [handleHeaderClick] 准备显示面板，当前值:', {
                        before: this.showCalibrationPanel,
                        headerClickCount: this.headerClickCount
                    });
                    ...
                    if (this.headerClickTimer) {
                        clearTimeout(this.headerClickTimer);
                        this.headerClickTimer = null;
                    }
                    
                    console.log('✅ [handleHeaderClick] 连续点击3次，已设置 showCalibrationPanel = true', {
                        showCalibrationPanel: this.showCalibrationPanel,
                        type: typeof this.showCalibrationPanel,
                        isTrue: this.showCalibrationPanel === true
                    });
                    
                    // 强制更新以确保组件显示
                    this.$forceUpdate();
                    this.$nextTick(() => {
                        console.log('🔍 [handleHeaderClick] nextTick 后检查:', {
                            showCalibrationPanel: this.showCalibrationPanel,
                            panelElements: document.querySelectorAll('[style*="打印校准设置"], h3').length,
...
let res = await api.phoneGetToFillDepOrders(requestData);
                ...
                if (res.data?.data) {
                    ...
                    console.log("📦 [ApplyPanel] phoneGetToFillDepOrders 金额字段:", {
                ...
                if (res.data && res.data.data) {
                    ...
                    if (hasSubDepartments && res.data.code === 0) {
                        ...
                        // 左侧点「子部门」时 nxDepFatherId 为父、nxDepId 为子；此前错误地把 arr 全部合并，打印会打成全店
                        ...
                        if (isSingleSubSelection) {
                            ...
                            if (depArrScoped.length === 0) {
                                console.warn(
                                    "[fetchOrderData] 当前选中子部门 depId=",
                                    depSel,
                                    "在接口 arr 中无匹配行，订单将为空（请核对后端 depId）"
                                );
                            } else {
                                console.log(
                                    "✅ [fetchOrderData] 子部门打印：仅保留 depId=",
                                    depSel,
                                    "共",
                                    depArrScoped.length,
                                    "段"
                        ...
                        if (isSingleSubSelection) {
                            const subNum = allOrders.reduce(
                                (sum, o) =>
                                    sum +
                                    parseFloat(
                                        o.nxDepartmentOrderSubTotal || o.nxDoSubtotal || 0
                                    ),
                                0
                            );
                        ...
                        console.log("========== [ApplyPanel] 子部门数据处理完成 ==========\n");
        ...
        async printOnly() {
            ...
            const orderRows = this._filteredRows.filter(r => !r.isDepartmentHeader);
            console.log("📊 [printOnly] 准备打印, 订单条数:", orderRows.length, "总页数:", this.printPagesData.length);
        ...
        async downLoadOnly() {
            ...
            // 添加表头
            excelData.push(['序号', '商品', '规格', '单位', '数量', '单价', '小计']);
...
.table-header {
    ...
    top: 0; /* 距离顶部0 */
    background-color: #f5f5f5; /* 背景色，防止内容透过 */
    z-index: 10; /* 确保表头在其他内容之上 */
    flex-shrink: 0; /* 表头不允许收缩 */
    }
