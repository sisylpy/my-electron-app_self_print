                            ...
                            <div style="flex: 2; min-width: 0; position: relative;">
                                ...
                                <div class="d-flex justify-content-between align-items-center p-2 border-bottom"
                                    ...
                                    <button class="btn btn-sm btn-link p-0"
                                            @click="$emit('close-before-order-search-results')"
                                            style="font-size: 14px; color: #856404; text-decoration: none;">
                                        ✕ 关闭
                                    </button>
                                </div>
                                    ...
                                    <div v-for="(goods, idx) in beforeOrderForm.disArr"
                                        ...
                                        <div class="d-flex">
                                            ...
                                            {{ goods.nxDgGoodsBrand }}
                                                    ...
                                                    <span v-else>
                                                    <span class="ms-2" v-if="goods.nxDgGoodsStandardWeight">
                                                ...
                                                <!-- 历史订货记录 -->
                                                <div v-if="goods && goods.departmentDisGoodsEntity !== null"
                                                     class="mt-2 d-flex"
                                                     style="margin-left: 24px; font-size: 12px; gap: 20px; flex-wrap: nowrap;">
                                                    <!--                                    时间-->
                                            ...
                                            <!-- 选择按钮 -->
                                            <div class="align-self-center" style="flex-shrink: 0; margin-left: 40px;">
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                isRecording: false,
                isStopping: false, // 停止录音锁，防止重复点击
                isStarting: false, // 开始录音锁，防止重复点击
                ...
                ttsAudioCache: {}, // TTS 音频缓存（index -> audioBase64）
                isTTSPlaying: false, // 是否正在播放（播放锁）
                currentTTSIndex: 0, // 当前播放的订单索引
                ttsSessionId: null, // TTS 会话 ID（统一前缀）
                ttsAudio: null, // 全局唯一的 audio 实例
                _ttsBound: false, // 防重复绑定标志
                stoppedIndex: -1, // 停止时的订单索引（用于继续朗读）
                orderItemRefs: {}, // 订单项的 DOM 引用
                ttsPlayerHeight: 0, // TTS播放器的高度（用于计算表头的 sticky top）
                previousOrderItemsLength: 0, // 上一次订单列表的长度（用于检测订单插入）
                ttsRequestCache: {}, // TTS 请求缓存（index -> Promise），用于请求去重
                ttsRequestRetries: {}, // TTS 请求重试次数（index -> count）
                ttsMaxRetries: 3, // 最大重试次数
                ttsRetryDelay: 1000, // 重试延迟（毫秒）
                ttsMaxConcurrent: 2, // 最大并发 TTS 请求数（Windows 下并发过多易出现 stream aborted）
                ttsActiveRequests: 0, // 当前进行中的 TTS 请求数
                goodsNameInputValues: {}, // 商品名称输入框的本地值（orderIndex -> value），用于延迟查询
                selectedMatchedGoodsIndex: {}, // 匹配商品列表选中索引 { orderIndex: index }
        ...
        mounted() {
            ...
            if (window.electronAPI) {
                window.electronAPI.onVoiceRecognitionResult((result) => {
                    console.log('[OrderList] 收到识别结果:', result);
                    this.recognitionResult = result;
                });

                window.electronAPI.onVoiceRecognitionError((error) => {
                    console.error('[OrderList] 识别错误:', error);
                    this.recognitionResult = {error: error};
                });

                // 监听 TTS 结果（仅用于预取缓存，不播放）
                window.electronAPI.onTTSResult((result) => {
                    console.log('[OrderList] 收到TTS结果（预取）:', result);
                    // 预取阶段只缓存，不播放
                    // 播放由 playNextInQueue 统一控制
                });

                window.electronAPI.onTTSError((error) => {
                    const errMsg = (error && error.error) ? String(error.error) : '';
                    // stream has been aborted 多为 Windows 下并发过多导致，渲染进程会重试，不更新 UI 状态
        ...
        beforeUnmount() {
            ...
            if (this.isRecording && window.electronAPI) {
                window.electronAPI.stopVoiceRecognition().catch(err => {
        ...
        methods: {
            ...
            async startRecording() {
                ...
                try {
                    ...
                    // ✅ 只发送开始命令，音频在主进程录制
                    if (window.electronAPI) {
                        const startResult = await window.electronAPI.startVoiceRecognition();
            ...
            async _fetchTTSWithRetry(index, text, retryCount = 0) {
                ...
                try {
                    ...
                    // 调用 TTS 接口
                    const result = await window.electronAPI.textToSpeech(text, this.ttsSessionId);
