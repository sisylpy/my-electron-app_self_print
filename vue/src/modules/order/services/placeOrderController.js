import api from '../services/orderApi';
    ...
    import ManualEntryUpload from '@/components/upload/ManualEntryUpload.vue';
    import PasteUpload from '@/components/upload/PasteUpload.vue';
    import ExcelPasteUpload from '@/components/upload/ExcelPasteUpload.vue';
    import AutoUpload from '@/components/upload/AutoUpload.vue';
    import OrderList from '../components/OrderEditor.vue';
    import DraftOrderList from '@/components/DraftOrderList.vue';
    import Toast from '@/components/Toast.vue';
    import AlertDialog from '@/components/AlertDialog.vue';
    import OrderModeSelector from '../components/OrderModeSelector.vue';
    import {createPlaceOrderState} from '../stores/orderWorkspaceState';
    ...
    export default {
        ...
        props: {
            ...
            initialUploadType: {
                type: String,
                default: 'manual',
                validator: value => ['manual', 'paste', 'image', 'excel-paste', 'excel', 'auto'].includes(value)
            },
            modeSelectorVariant: {
                type: String,
                default: 'inline',
                validator: value => ['inline', 'cards'].includes(value)
            },
            showModeSelector: {
                type: Boolean,
                default: true
        ...
        data() {
            const state = createPlaceOrderState();
            state.uploadType = this.initialUploadType;
            state.excelPasteEverShown = this.initialUploadType === 'excel-paste';
            return state;
        },
        computed: {
        ...
        methods: {
            ...
            async pastSavePasteOrders() {
                ...
                try {
                    ...
                    await this.$refs.alertDialog.alert('保存订单失败，请重试：' + serverMessage + requestHint, 'error');
                } finally {
...
请严格按照上述格式输出，只输出 JSON 数组，不要添加任何其他内容。`;
                    ...
                    throw new Error(
                        '客户端直连 DeepSeek 已停用。请先在 nongxinle-server 部署受鉴权的文本优化代理接口。'
                    );
                } catch (error) {
                    throw error;
                }
...
- 只输出 JSON，不要输出任何其他文字。`;
                    ...
                    throw new Error(
                        '客户端直连 DeepSeek 已停用。请先在 nongxinle-server 部署受鉴权的 Excel 文本解析代理接口。'
                    );
                } catch (error) {
                    throw error;
                }
