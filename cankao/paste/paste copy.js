...
Page({
  ...
  async aiRecogniseFirst() {
    ...
    try {
      const optimizedText = await optimizeTextWithDeepSeek(content, 0.2, this.data.brandPrompts);
   ...
   */
  async againAi() {
    // 优先使用 inputContent（用户可能修改了输入框内容）
    // 如果没有 inputContent，再使用 originSentence（原始内容）
    const content = this.data.inputContent || this.data.originSentence || this.data.sentence;
    let temp = 0.1; // 使用低温以获得更稳定的结果
    let retry = this.data.aiRetryCount || 0;
    ...
    console.log('开始第二次 AI 识别，输入内容:', content, '尝试次数:', retry + 1);
    ...
    try {
      const optimizedText = await optimizeTextWithDeepSeek(content, temp, this.data.brandPrompts);
      console.log('第二次 AI 识别完成，优化后内容:', optimizedText);
  ...
  onUnload() {
    ...
    if (temp.length == 0) {
      console.log("没有 status == -2 的订单，准备删除缓存");
      
      var arr = pasteDepList.filter(item => item.depId !== this.data.pasteDepId);
      console.log("过滤后的 arr 长度:", arr.length);
      console.log("过滤后的 arr 内容:", arr);

      if (arr.length == 0) {
        console.log("arr 为空，删除整个 pasteDepList 缓存");
        wx.removeStorageSync('pasteDepList');
        console.log("已删除 pasteDepList 缓存");
        
        // 验证删除是否成功
        var verifyStorage = wx.getStorageSync('pasteDepList');
        console.log("验证：删除后的缓存内容:", verifyStorage);
      } else {
        console.log("保存更新后的 pasteDepList 到缓存");
        try {
        wx.setStorageSync('pasteDepList', arr);
        console.log("已保存到缓存，缓存内容:", arr);
        } catch (error) {
          console.error('[删除未保存订单] 缓存同步失败:', error);
          if (error.message && error.message.includes('exceed storage item max length')) {
        ...
        // 验证保存是否成功
        var verifyStorage = wx.getStorageSync('pasteDepList');
        console.log("验证：保存后的缓存内容:", verifyStorage);
      }
    } else {
